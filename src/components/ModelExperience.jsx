...
function ModalExperience(props) {
  ...
  const handleFileSaving = (e) => {
    ...
    form_data.append("experienceImage", file);
  ...
  const handFileUpload = async (data) => {
    try {
      if (formData !== undefined) {
        let newRes = await fetch(url + `/${data.id}/uploadExperienceImage`, {
          method: "POST",
  ...
  const handleDelete = async (e) => {
    e.preventDefault();
    try {
      let response = await fetch(url + "/" + props.item.id, {
        method: "DELETE",
        headers: {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    let method = "POST";
    let fetchUrl = url + "/" + userId;
    if (props.item?.id) {
      method = "PUT";
      fetchUrl = url + "/" + props.item.id;
    }
    try {
